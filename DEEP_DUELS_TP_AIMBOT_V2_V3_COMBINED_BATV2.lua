...
-- ============================================================
-- MAY ANIMATION PACKS - portados para o MAIN do Deep
-- ============================================================
_G.DeepAnimPack = _G.DeepAnimPack or {}
_G.DeepAnimPack.selected = "OFF"
_G.DeepAnimPack.index = 1
_G.DeepAnimPack.file = "DeepDuels_AnimPack.txt"
_G.DeepAnimPack.list = {
    "OFF","Unwalk","Hit Harder","Zombie","Ninja","Knight","Elder",
    "Levitate","Astronaut","Pirate","Toy","Vampire","Werewolf","Rthro","Stylish"
}
_G.DeepAnimPack.packs = {
...
_G.DeepAnimPack.original = {}

function _G.DeepAnimPack.getAnimate(char)
    char=char or LP.Character
    return char and char:FindFirstChild("Animate") or nil
end

function _G.DeepAnimPack.stopTracks(char)
    local hum=char and char:FindFirstChildOfClass("Humanoid")
...
function _G.DeepAnimPack.setId(obj,id)
    if obj and id then pcall(function() obj.AnimationId=id end) end
end

function _G.DeepAnimPack.reload(animate)
...
function _G.DeepAnimPack.backup(char)
    local animate=_G.DeepAnimPack.getAnimate(char)
    if not animate or next(_G.DeepAnimPack.original)~=nil then return end
    local function gid(obj) return obj and obj.AnimationId or nil end
    _G.DeepAnimPack.original={
        idle1=gid(animate.idle and animate.idle:FindFirstChild("Animation1")),
        idle2=gid(animate.idle and animate.idle:FindFirstChild("Animation2")),
        walk=gid(animate.walk and animate.walk:FindFirstChild("WalkAnim")),
        run=gid(animate.run and animate.run:FindFirstChild("RunAnim")),
        jump=gid(animate.jump and animate.jump:FindFirstChild("JumpAnim")),
        fall=gid(animate.fall and animate.fall:FindFirstChild("FallAnim")),
        climb=gid(animate.climb and animate.climb:FindFirstChild("ClimbAnim")),
    }
end

function _G.DeepAnimPack.reset()
    local char=LP.Character
    local animate=_G.DeepAnimPack.getAnimate(char)
    if not animate or next(_G.DeepAnimPack.original)==nil then return end
    _G.DeepAnimPack.stopTracks(char)
    local o=_G.DeepAnimPack.original
    _G.DeepAnimPack.setId(animate.idle and animate.idle:FindFirstChild("Animation1"),o.idle1)
    _G.DeepAnimPack.setId(animate.idle and animate.idle:FindFirstChild("Animation2"),o.idle2)
    _G.DeepAnimPack.setId(animate.walk and animate.walk:FindFirstChild("WalkAnim"),o.walk)
    _G.DeepAnimPack.setId(animate.run and animate.run:FindFirstChild("RunAnim"),o.run)
    _G.DeepAnimPack.setId(animate.jump and animate.jump:FindFirstChild("JumpAnim"),o.jump)
    _G.DeepAnimPack.setId(animate.fall and animate.fall:FindFirstChild("FallAnim"),o.fall)
    _G.DeepAnimPack.setId(animate.climb and animate.climb:FindFirstChild("ClimbAnim"),o.climb)
    _G.DeepAnimPack.reload(animate)
end

function _G.DeepAnimPack.save(pack)
    pcall(function()
        if writefile then writefile(_G.DeepAnimPack.file,tostring(pack or "OFF")) end
    end)
end

function _G.DeepAnimPack.valid(pack)
    for _,name in ipairs(_G.DeepAnimPack.list) do
        if name==pack then return true end
    end
    return false
end

function _G.DeepAnimPack.load()
    local pack=nil
    pcall(function()
        if isfile and readfile and isfile(_G.DeepAnimPack.file) then
            pack=tostring(readfile(_G.DeepAnimPack.file) or "")
            pack=pack:gsub("^%s+",""):gsub("%s+$","")
        end
    end)
    if not _G.DeepAnimPack.valid(pack) then pack="OFF" end
    _G.DeepAnimPack.selected=pack
    for i,name in ipairs(_G.DeepAnimPack.list) do
        if name==pack then _G.DeepAnimPack.index=i break end
    end
    return pack
end

function _G.DeepAnimPack.apply(packName)
    if not _G.DeepAnimPack.valid(packName) then packName="OFF" end

    local previous=_G.DeepAnimPack.selected
    _G.DeepAnimPack.selected=packName
    _G.DeepAnimPack.save(packName)

    -- Sai do Unwalk antes de aplicar qualquer outro pack.
    if packName~="Unwalk" and (unwalkEnabled or previous=="Unwalk") then
        unwalkEnabled=false
        pcall(stopUnwalk)
    end
    ...
    if packName=="Unwalk" then
        if animate then
            _G.DeepAnimPack.backup(char)
            _G.DeepAnimPack.reset()
        end
        unwalkEnabled=true
        pcall(startUnwalk)
        return true
    ...
    if packName=="Hit Harder" then
        local h=_G.DeepAnimPack.hitHarder
        _G.DeepAnimPack.setId(animate.idle and animate.idle:FindFirstChild("Animation1"),h.idle1)
        _G.DeepAnimPack.setId(animate.idle and animate.idle:FindFirstChild("Animation2"),h.idle2)
        _G.DeepAnimPack.setId(animate.walk and animate.walk:FindFirstChild("WalkAnim"),h.walk)
        _G.DeepAnimPack.setId(animate.run and animate.run:FindFirstChild("RunAnim"),h.run)
        _G.DeepAnimPack.setId(animate.jump and animate.jump:FindFirstChild("JumpAnim"),h.jump)
        _G.DeepAnimPack.setId(animate.fall and animate.fall:FindFirstChild("FallAnim"),h.fall)
        _G.DeepAnimPack.reload(animate)
        return true
    end

    local pack=_G.DeepAnimPack.packs[packName]
    if not pack then return false end
    _G.DeepAnimPack.setId(animate.idle and animate.idle:FindFirstChild("Animation1"),pack.idle[1][1])
    _G.DeepAnimPack.setId(animate.idle and animate.idle:FindFirstChild("Animation2"),pack.idle[2][1])
    _G.DeepAnimPack.setId(animate.walk and animate.walk:FindFirstChild("WalkAnim"),pack.walk)
    _G.DeepAnimPack.setId(animate.run and animate.run:FindFirstChild("RunAnim"),pack.run)
    _G.DeepAnimPack.setId(animate.jump and animate.jump:FindFirstChild("JumpAnim"),pack.jump)
    _G.DeepAnimPack.setId(animate.fall and animate.fall:FindFirstChild("FallAnim"),pack.fall)
    _G.DeepAnimPack.setId(animate.climb and animate.climb:FindFirstChild("ClimbAnim"),pack.climb)
    _G.DeepAnimPack.reload(animate)
    return true
end

function _G.DeepAnimPack.setIndex(idx)
    if idx<1 then idx=#_G.DeepAnimPack.list end
    if idx>#_G.DeepAnimPack.list then idx=1 end
    _G.DeepAnimPack.index=idx
    local pack=_G.DeepAnimPack.list[idx]
    _G.DeepAnimPack.apply(pack)
    if _G.DeepAnimPack.refresh then pcall(_G.DeepAnimPack.refresh) end
    return pack
end

_G.DeepAnimPack.load()
...
LP.CharacterAdded:Connect(function(char)
    task.defer(function()
        ...
        local pack=_G.DeepAnimPack.selected or "OFF"
        if pack~="OFF" then pcall(function() _G.DeepAnimPack.apply(pack) end) end
    end)
end)
...
tpAimbotMode = tpAimbotMode or "v2"
if tpAimbotMode~="v2" and tpAimbotMode~="v3" then tpAimbotMode="v2" end

antiTpAimbotEnabled = antiTpAimbotEnabled or false
setAntiTpAimbotVisual = setAntiTpAimbotVisual or nil

-- ============================================================
-- MAY DUELS TP AIMBOT CORE
-- V1/V2/V3 portados do May e adaptados somente aos controles
-- já existentes do Deep Duels.
-- ============================================================
_G.DeepMayTPBatNames = _G.DeepMayTPBatNames or {
    "Bat","Slap","Iron Slap","Gold Slap","Diamond Slap",
    "Emerald Slap","Ruby Slap","Dark Matter Slap","Flame Slap",
    "Nuclear Slap","Galaxy Slap","Glitched Slap"
}

function _G.DeepMayMakeRuntime()
    return {
        conn=nil,
...
_G.DeepMayTPMain = _G.DeepMayTPMain or _G.DeepMayMakeRuntime()
_G.DeepMayAntiTP = _G.DeepMayAntiTP or _G.DeepMayMakeRuntime()

function _G.DeepMayFiniteVector3(v)
    return v and v.X==v.X and v.Y==v.Y and v.Z==v.Z
        and math.abs(v.X)<1e7 and math.abs(v.Y)<1e7 and math.abs(v.Z)<1e7
end

function _G.DeepMayGetBat()
    local char=LP.Character
    if not char then return nil end

    for _,name in ipairs(_G.DeepMayTPBatNames) do
...
function _G.DeepMayTrySwing(rt)
    if not rt or rt.hittingCooldown then return end
    rt.hittingCooldown=true
    pcall(function()
        local char=LP.Character
        if not char then return end
        local bat=_G.DeepMayGetBat()
    ...
    task.delay(0.08,function()
        if rt then rt.hittingCooldown=false end
    end)
end

function _G.DeepMayInspectTarget(rt,player,targetRoot,myRoot)
    if not rt or not player or not targetRoot or not targetRoot.Parent then return true,nil end
    rt.targetSamples=rt.targetSamples or {}
    local sample=rt.targetSamples[player] or {}
    rt.targetSamples[player]=sample
    ...
    local badVelocity=not _G.DeepMayFiniteVector3(vel) or vel.Magnitude>650 or math.abs(vel.Y)>150
    local badPosition=not _G.DeepMayFiniteVector3(pos) or pos.Y<=voidFloor or pos.Y>650
...
function _G.DeepMayGuardVoid(rt,root,hum,targetRoot)
    if not rt or not root or not hum then return false end
    ...
    local dt=now-(rt.lastSampleTime or now)
    local suddenDrop=rt.lastPosition and dt<=0.35 and position.Y<rt.lastPosition.Y-40
    local suddenRise=rt.lastPosition and dt<=0.35 and position.Y>rt.lastPosition.Y+25
    local safeRise=rt.lastSafeCFrame and position.Y>rt.lastSafeCFrame.Position.Y+30
    ...
    local recovering=now<(rt.voidRecoverUntil or 0)
    ...
    if inVoid or suddenDrop or suddenRise or safeRise or forcedLaunch or resetHeight then
        ...
        local recoveryCFrame=rt.safeEngagementCFrame or rt.lastSafeCFrame
    ...
    rt.lastPosition=root.Position
    rt.lastSampleTime=now
    return recovering
end

function _G.DeepMayGetClosestPlayer(rt,useLocked)
    ...
    if useLocked then
        ...
        if locked and locked.Parent==Players and locked.Character then
            ...
            if lr and lh and lh.Health>0 then return locked,lr end
        end
    end
    ...
    for _,player in ipairs(Players:GetPlayers()) do
        if player~=LP and player.Character then
            ...
            if root and hum and hum.Health>0 then
                local comparePos=root.Position
                if useLocked then
                    local suspicious,sample=_G.DeepMayInspectTarget(rt,player,root,hrp)
                    if suspicious and sample and sample.safePosition then
                        comparePos=sample.safePosition
                    end
                end
                local dist=(hrp.Position-comparePos).Magnitude
    ...
    rt.targetPlayer=best
    return best,bestRoot
end

function _G.DeepMaySyncTargetRefs(rt,target)
    if rt==_G.DeepMayTPMain then
...
function _G.DeepMayPrepareRuntime(rt,mode)
    if rt.conn then
        pcall(function() rt.conn:Disconnect() end)
        rt.conn=nil
    end

    rt.enabled=true
    rt.mode=mode
    rt.hittingCooldown=false
    rt.targetPlayer=nil
    rt.safeEngagementCFrame=nil
    rt.targetSamples=rt.targetSamples or {}

    local char=LP.Character
    rt.h=char and char:FindFirstChildOfClass("Humanoid") or nil
    rt.hrp=char and char:FindFirstChild("HumanoidRootPart") or nil

    if rt.h then
        rt.prevAutoRotate=rt.h.AutoRotate
    end

    if rt.hrp and rt.hrp.Position.Y>-30 then
        rt.lastSafeCFrame=rt.hrp.CFrame
        rt.lastPosition=rt.hrp.Position
        rt.lastSampleTime=tick()
    end
end

function _G.DeepMayRefreshCharacter(rt)
    local char=LP.Character
    if not char then return nil,nil end
    if not rt.h or not rt.h.Parent then
        rt.h=char:FindFirstChildOfClass("Humanoid")
    end
    if not rt.hrp or not rt.hrp.Parent then
        rt.hrp=char:FindFirstChild("HumanoidRootPart")
    end
    return rt.h,rt.hrp
end

function _G.DeepMayStartV1(rt)
    _G.DeepMayPrepareRuntime(rt,"v1")

    rt.conn=RunService.Heartbeat:Connect(function()
        if not rt.enabled then return end
        local hum,hrp=_G.DeepMayRefreshCharacter(rt)
        if not hum or not hrp then return end

        local target,tr=_G.DeepMayGetClosestPlayer(rt,true)
        _G.DeepMaySyncTargetRefs(rt,target)
        if not target or not tr then return end

        local suspicious,sample=_G.DeepMayInspectTarget(rt,target,tr,hrp)
        if _G.DeepMayGuardVoid(rt,hrp,hum,suspicious and nil or tr) then
            _G.DeepMayTrySwing(rt)
            return
        end

        if suspicious then
            local recovery=rt.safeEngagementCFrame or rt.lastSafeCFrame
            if sethiddenproperty then
            ...
            if _G.DeepMayFiniteVector3(tr.Position) then
            ...
            pcall(function()
                ...
                if (hrp.Position-sanitizedPos).Magnitude>5 then
                    hrp.CFrame=sanitizedCFrame
                end
            end)

            local cam=workspace.CurrentCamera
            if cam then
                cam.CFrame=CFrame.new(cam.CFrame.Position,sanitizedPos)
            end
            _G.DeepMayTrySwing(rt)
            return
        end
        ...
        rt.safeEngagementCFrame=CFrame.lookAt(targetPos,targetPos+facing.Unit)

        pcall(function()
            hrp.CFrame=rt.safeEngagementCFrame
            hrp.AssemblyLinearVelocity=Vector3.new(tvel.X,0,tvel.Z)
            hrp.AssemblyAngularVelocity=Vector3.zero
        end)

        if sethiddenproperty and now>=(rt._physPulseUntil or 0) then
...
function _G.DeepMayStartV2(rt)
    _G.DeepMayPrepareRuntime(rt,"v2")
    ...
    rt.conn=RunService.Heartbeat:Connect(function()
        ...
        local target,tr=_G.DeepMayGetClosestPlayer(rt,false)
        _G.DeepMaySyncTargetRefs(rt,target)
        if not target or not tr then return end

        local behind=tr.CFrame*CFrame.new(0,0,3.2)
        if (hrp.Position-behind.Position).Magnitude>4 then
            hrp.CFrame=CFrame.new(behind.Position,tr.Position)
        end
...
function _G.DeepMayStartV3(rt)
    _G.DeepMayPrepareRuntime(rt,"v3")
    ...
    rt.conn=RunService.Heartbeat:Connect(function()
        ...
        if sethiddenproperty then
            pcall(sethiddenproperty,hrp,"PhysicsRepRootPart",tr)
        end
...
function _G.DeepMayStartRuntime(rt,mode)
    if mode=="v3" then return _G.DeepMayStartV3(rt) end
    if mode=="v2" then return _G.DeepMayStartV2(rt) end
    return _G.DeepMayStartV1(rt)
end

function _G.DeepMayStopRuntime(rt)
    ...
    local hum=char and char:FindFirstChildOfClass("Humanoid")
    if hum then
        hum.AutoRotate=(rt.prevAutoRotate==nil) and true or rt.prevAutoRotate
        hum.PlatformStand=false
        pcall(function() hum:ChangeState(Enum.HumanoidStateType.GettingUp) end)
    end
...
-- Mantém as APIs já usadas pelo Deep.
_G.DeepTPAimbot=_G.DeepMayTPMain
_G.DeepTPAimbotOn=_G.DeepTPAimbotOn or false

function _G.DeepTPAimbotGetBat()
    return _G.DeepMayGetBat()
end

function _G.DeepTPAimbotTrySwing()
    return _G.DeepMayTrySwing(_G.DeepMayTPMain)
end

function _G.DeepTPAimbotGetClosestPlayer()
    local p,r=_G.DeepMayGetClosestPlayer(_G.DeepMayTPMain,false)
    if not p then return nil,math.huge end
    return p,(r and _G.DeepMayTPMain.hrp) and (_G.DeepMayTPMain.hrp.Position-r.Position).Magnitude or math.huge
end

function _G.DeepTPAimbotSetupChar(char)
    if not char then return end
    _G.DeepMayTPMain.h=char:FindFirstChildOfClass("Humanoid")
    _G.DeepMayTPMain.hrp=char:FindFirstChild("HumanoidRootPart")
end

function _G.DeepStartTPAimbot()
    _G.DeepTPAimbotOn=true
    tpAimbotEnabled=true
    local ok=_G.DeepMayStartRuntime(_G.DeepMayTPMain,tpAimbotMode)
    if ok==false then
...
function _G.DeepStopTPAimbot()
    _G.DeepTPAimbotOn=false
    tpAimbotEnabled=false
    _G.DeepMayStopRuntime(_G.DeepMayTPMain)
end
...
function setTPAimbotMode(mode)
    if mode~="v2" and mode~="v3" then mode="v2" end
    if tpAimbotMode==mode then
        if _G.DeepRefreshTPAimbotMode then pcall(_G.DeepRefreshTPAimbotMode) end
        if _G.DeepRefreshTPAimbotLabels then pcall(_G.DeepRefreshTPAimbotLabels) end
        return
    end

    local wasOn=tpAimbotEnabled==true
    if wasOn then _G.DeepStopTPAimbot() end
    tpAimbotMode=mode
    if _G.DeepRefreshTPAimbotMode then pcall(_G.DeepRefreshTPAimbotMode) end
    if _G.DeepRefreshTPAimbotLabels then pcall(_G.DeepRefreshTPAimbotLabels) end
    if wasOn then _G.DeepStartTPAimbot() end
    pcall(saveConfig)
end
...
function setTPAimbot(on)
    on=on==true
    if on then
        -- Um único botão liga os dois motores juntos:
        -- TP Aimbot V2/V3 + antigo Bat V2 (oculto na interface).
        startAimbot2()
    ...
    if _G.DeepRefreshTPAimbotLabels then
        pcall(_G.DeepRefreshTPAimbotLabels)
    end
    return true
end

-- ============================================================
-- ANTI TP AIMBOT - motor ANTI BAT TP do MAY DUELS
-- ============================================================
DeepMayAntiBatTp = DeepMayAntiBatTp or { enabled=false, myHRP=nil, fakeHRP=nil, conn=nil }
DEEP_MAY_ANTI_BAT_TP_FAKE_Y = -1000

function deepMayAntiBatTpStop()
    if DeepMayAntiBatTp.conn then
        DeepMayAntiBatTp.conn:Disconnect()
        DeepMayAntiBatTp.conn=nil
    end
    if DeepMayAntiBatTp.fakeHRP then
        pcall(function() DeepMayAntiBatTp.fakeHRP:Destroy() end)
        DeepMayAntiBatTp.fakeHRP=nil
    end
    pcall(function()
        if sethiddenproperty and DeepMayAntiBatTp.myHRP and DeepMayAntiBatTp.myHRP.Parent then
            sethiddenproperty(DeepMayAntiBatTp.myHRP,"PhysicsRepRootPart",DeepMayAntiBatTp.myHRP)
        end
    end)
end

function deepMayAntiBatTpStart()
    deepMayAntiBatTpStop()

    if not DeepMayAntiBatTp.myHRP then
        local ch=LP.Character
        if ch then DeepMayAntiBatTp.myHRP=ch:FindFirstChild("HumanoidRootPart") end
    end
    if not DeepMayAntiBatTp.myHRP then return false end

    local fake=Instance.new("Part")
    fake.Name="BlodyAntiBatFakeHRP"
    fake.Size=Vector3.new(2,2,1)
    fake.Anchored=true
    fake.CanCollide=false
    fake.Transparency=1
    fake.CFrame=CFrame.new(
        DeepMayAntiBatTp.myHRP.Position.X,
        DEEP_MAY_ANTI_BAT_TP_FAKE_Y,
        DeepMayAntiBatTp.myHRP.Position.Z
    )
    fake.Parent=workspace
    DeepMayAntiBatTp.fakeHRP=fake

    DeepMayAntiBatTp.conn=RunService.Heartbeat:Connect(function()
        if not DeepMayAntiBatTp.enabled then return end
        local myHRP,fakeHRP=DeepMayAntiBatTp.myHRP,DeepMayAntiBatTp.fakeHRP
...
function deepMayAntiBatTpSetupChar(char)
    task.wait(0.2)
    DeepMayAntiBatTp.myHRP=char:WaitForChild("HumanoidRootPart",5)
    if DeepMayAntiBatTp.enabled and DeepMayAntiBatTp.myHRP then
        deepMayAntiBatTpStop()
        task.wait(0.1)
        deepMayAntiBatTpStart()
    end
end

LP.CharacterAdded:Connect(function(char)
    deepMayAntiBatTpStop()
    task.spawn(function() deepMayAntiBatTpSetupChar(char) end)
end)
if LP.Character then
    task.spawn(function() deepMayAntiBatTpSetupChar(LP.Character) end)
end

function setAntiTpAimbot(on)
    antiTpAimbotEnabled=(on==true)
    DeepMayAntiBatTp.enabled=antiTpAimbotEnabled

    if antiTpAimbotEnabled then
        local ok=deepMayAntiBatTpStart()
        if ok==false then
            antiTpAimbotEnabled=false
            DeepMayAntiBatTp.enabled=false
        end
    else
        deepMayAntiBatTpStop()
    end
    ...
    if setAntiTpAimbotVisual then
        pcall(function() setAntiTpAimbotVisual(antiTpAimbotEnabled) end)
    end
    if mobBtnRefs and mobBtnRefs.antiTpAimbot then
        pcall(function() mobBtnRefs.antiTpAimbot(antiTpAimbotEnabled) end)
    end
...
mobBtnTextRefs=mobBtnTextRefs or {} 
...
function buildMobileButtons() 
    ...
    local defs={ 
        ...
        {top="TP",bot=(tpAimbotMode=="v3" and "AIMBOT V3" or "AIMBOT V2"),key="antiDesync",layout="stacked"},
    ...
    for i,def in ipairs(defs) do 
        ...
        mobBtnTextRefs[def.key]={top=top,bot=bot}
        if def.key=="antiDesync" then
            _G.DeepRefreshTPAimbotLabels=function()
                local modeText=(tpAimbotMode=="v3") and "AIMBOT V3" or "AIMBOT V2"
                local ref=mobBtnTextRefs and mobBtnTextRefs.antiDesync
                if ref then
                    if ref.top then ref.top.Text="TP" end
                    if ref.bot then ref.bot.Text=modeText end
                end
                if _G.DeepTPAimbotMenuLabel and _G.DeepTPAimbotMenuLabel.Parent then
                    _G.DeepTPAimbotMenuLabel.Text=(tpAimbotMode=="v3") and "TP AIMBOT V3" or "TP AIMBOT V2"
                end
            end
            pcall(_G.DeepRefreshTPAimbotLabels)
        end
        ...
        hit.InputBegan:Connect(function(input) 
            if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then 
                local freeOrangeButton = (def.key=="antiDesync")
                -- Lock UI trava o TP AIMBOT.
                if uiLocked then return end
                -- TP AIMBOT pode mover livremente enquanto a UI estiver destravada.
...
function saveConfig() 
    ...
    local cfg={ 
        ...
        autoBat=autoBatEnabled,autoSwing=autoSwingEnabled,autoBatSpeed=AUTO_BAT_SPEED,aimbot2=aimbot2Enabled,antiDesyncAimbot=tpAimbotEnabled,tpAimbotMode=tpAimbotMode,         espEnabled=_G.DeepESP.enabled,espLine=_G.DeepESP.line,espSquare=_G.DeepESP.square,espExpanded=_G.DeepESP.expanded,
...
function buildDeepAnimationPackControl(page,mkRow,mkLabel)
    local row=mkRow(page,38)
    mkLabel(row,"Animation Pack")

    local left=Instance.new("TextButton",row)
    left.Size=UDim2.new(0,24,0,22)
    left.Position=UDim2.new(1,-150,0.5,-11)
    left.BackgroundColor3=Color3.fromRGB(10,10,14)
    left.BorderSizePixel=0
    left.Text="<"
    left.TextColor3=Color3.fromRGB(255,255,255)
    left.Font=Enum.Font.SciFi
    left.TextSize=12
    left.ZIndex=5
    Instance.new("UICorner",left).CornerRadius=UDim.new(0,5)
    local ls=Instance.new("UIStroke",left)
    ls.Color=Color3.fromRGB(80,80,90)
    ls.Thickness=1

    local value=Instance.new("TextLabel",row)
    value.Size=UDim2.new(0,92,0,22)
    value.Position=UDim2.new(1,-122,0.5,-11)
    value.BackgroundColor3=Color3.fromRGB(10,10,14)
    ...
    value.ZIndex=5
    Instance.new("UICorner",value).CornerRadius=UDim.new(0,5)
    local vs=Instance.new("UIStroke",value)
    vs.Color=Color3.fromRGB(45,45,55)
    vs.Thickness=1

    local right=Instance.new("TextButton",row)
    right.Size=UDim2.new(0,24,0,22)
    right.Position=UDim2.new(1,-26,0.5,-11)
    right.BackgroundColor3=Color3.fromRGB(10,10,14)
    right.BorderSizePixel=0
    right.Text=">"
    right.TextColor3=Color3.fromRGB(255,255,255)
    right.Font=Enum.Font.SciFi
    right.TextSize=12
    right.ZIndex=5
    Instance.new("UICorner",right).CornerRadius=UDim.new(0,5)
    local rs=Instance.new("UIStroke",right)
    rs.Color=Color3.fromRGB(80,80,90)
    rs.Thickness=1

    _G.DeepAnimPack.refresh=function()
        value.Text=tostring(_G.DeepAnimPack.selected or "OFF"):upper()
    end
    _G.DeepAnimPack.refresh()

    left.Activated:Connect(function()
        if _anyKeyListening then return end
        local pack=_G.DeepAnimPack.setIndex((_G.DeepAnimPack.index or 1)-1)
        if setUnwalkVisual then setUnwalkVisual(pack=="Unwalk") end
    end)
...
local function buildGui() 
    ...
    buildDeepAnimationPackControl(menuPage,mkRow,mkLabel)
    mkSect(menuPage,"ESP")
    do
    ...
    do
        local row=mkRow(aimbotPage,32)
        local label=Instance.new("TextLabel",row)
        label.Size=UDim2.new(0.62,0,1,0);label.Position=UDim2.new(0,9,0,0)
        label.BackgroundTransparency=1
        label.Text=(tpAimbotMode=="v3") and "TP AIMBOT V3" or "TP AIMBOT V2"
        label.TextColor3=W;label.ZIndex=4
        label.Font=Enum.Font.SciFi;label.TextSize=10;label.TextXAlignment=Enum.TextXAlignment.Left
        _G.DeepTPAimbotMenuLabel=label

        local pill,dot=mkPill(row,42)
        local on=false
        local function sv(s) on=s;animPill(pill,dot,s) end
        setTPAimbotVisual=sv
        local clk=Instance.new("TextButton",pill)
        clk.Size=UDim2.new(1,0,1,0);clk.BackgroundTransparency=1;clk.Text="";clk.ZIndex=5
        clk.Activated:Connect(function()
            if _anyKeyListening then return end
            setTPAimbot(not tpAimbotEnabled)
            sv(tpAimbotEnabled)
        end)
        if _G.DeepRefreshTPAimbotLabels then pcall(_G.DeepRefreshTPAimbotLabels) end
    end
    do
        local row=mkRow(aimbotPage,32);mkLabel(row,"TP Aimbot Mode")
        local refs={}
        local modes={{id="v2",txt="V2"},{id="v3",txt="V3"}}

        local function refreshTPMode()
            for _,r in ipairs(refs) do
                local active=(tpAimbotMode==r.id)
        ...
        _G.DeepRefreshTPAimbotMode=refreshTPMode

        for i,m in ipairs(modes) do
            local btn=Instance.new("TextButton",row)
            btn.Size=UDim2.new(0,58,0,22)
            btn.Position=UDim2.new(1,-(122-(i-1)*62),0.5,-11)
            ...
            refs[#refs+1]={id=m.id,btn=btn}
            btn.Activated:Connect(function()
                setTPAimbotMode(m.id)
                refreshTPMode()
            end)
        end
        refreshTPMode()
...
local function loadConfigKeys() 
    ...
    if cfg.tpAimbotMode=="v2" or cfg.tpAimbotMode=="v3" then tpAimbotMode=cfg.tpAimbotMode else tpAimbotMode="v2" end
...
local function loadConfigState() 
    ...
    task.spawn(function() 
        ...
        -- Bat V2 fica oculto e só é ligado junto com o TP Aimbot V2/V3.
        stopAimbot2()
